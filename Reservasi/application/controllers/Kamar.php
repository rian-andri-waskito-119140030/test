...
class Kamar extends CI_Controller {
	...
	public function read()
	{
		...
		$data['lapangan']=$this->Kamar_model->read();
		$data['error'] = '';
	...
	public function edit($id)
	{
        ...
        $data['detail'] = $this->db->get_where('lapangan',['id' => $id])->row();
	...
	public function do_upload()
    {
	    ...
	    if (!$this->upload->do_upload('gambar')){
	        ...
	        $query = $this->db->insert('lapangan',$data);
	        if($query){
	...
	public function update()
	{
        ...
        $_image = $this->db->get_where('lapangan',['id' => $id])->row();
        $config['upload_path']          = './images/kamar/';
        $config['allowed_types']        = 'jpg|png';
        $config['max_size']             = 0;
        $config['max_width']            = 0;
        ...
        if (!$this->upload->do_upload('gambar')){
            ...
            $query = $this->db->update('lapangan', $data, array('id' => $id));
        }
        else{
            $_data = array('upload_data' => $this->upload->data());
             $data = array(
